...
public class ParametriClimatici {
    ...
    /**
    *@ param boolean mostra, mostra i dati dei parametri in caso di visualizzazione dei dati
    *@param File[] files directory dei file
    * Dato in input il percorso dei file .dati visualizza tutti i dati delle aree esistenti
    * nella directory mostrandone i dati da inserire per visualizzare i dati e aggiorna il file ParametriClimatici.dati
    */
    public void datiparametri(File[] files, boolean mostra) throws IOException {
        File f = new File("C:\\Climate Monitoring"+File.separator+"ParametriClimatici.dati");
        BufferedWriter bf = new BufferedWriter(new FileWriter(f));
        for (File file : files) {
            if (file.isDirectory()) {
                datiparametri(file.listFiles(), mostra);
            } else {
                String name = file.getName();
                //System.out.println(name);
                String[] dati_nome = name.split("------");
                String[] no_ext = dati_nome[2].split(".txt");
                if(mostra){
                    System.out.println("Geoname ID\tData delle informazioni\tAggiunte dal centro");
                    System.out.println(dati_nome[0]+"\t"+dati_nome[1].charAt(0)+dati_nome[1].charAt(1)+"/"+dati_nome[1].charAt(2)+dati_nome[1].charAt(3)+"/"+dati_nome[1].charAt(4)+dati_nome[1].charAt(5)+dati_nome[1].charAt(6)+dati_nome[1].charAt(7)+"\t"+no_ext[0]);
                }
                
                bf.write(dati_nome[0]+"\t"+dati_nome[1]+"\t"+no_ext[0]+"\n");
    ...
    /**
    *  Mostra i report esistenti
    */
    public void visualizza_disponibili(boolean mostra) throws IOException{
        File f = new File("C:\\Climate Monitoring"+File.separator+"ParametriClimatici.dati");
        File dir = new File("C:\\Climate Monitoring"+File.separator+"Parametri");
        this.datiparametri(dir.listFiles(), mostra);
    }
    ...
    public void visualizzaAreaGeografica(String geonameid, String data) throws FileNotFoundException, IOException{
        ...
        File dir_e = new File("C:\\Climate Monitoring"+File.separator+"Parametri\\");
        ...
        for (File file : dir_e.listFiles()) {
            String nome = file.getName();
            //System.out.println(nome);
            //System.out.println(file.getAbsolutePath());
            if(nome.contains(geonameid+"------"+dati_data[0]+dati_data[1]+dati_data[2])){
                parametri = new File(file.getAbsolutePath());
        ...
        Scanner scanf = new Scanner(f);
            ...
            switch(scelta+1){
                ...
                case 8:
                    ...
                    this.visualizza_disponibili(false);
     ...
     * @throws IOException
     */
    public void inserisciParametriClimatici(String geonameid, String data, String vento, String umidita, String pressione, String temperatura, String precipitazioni, String altitudine_ghiacciai, String massa_ghiacciai, String centro) throws IOException{
        String[] dati_data = data.split("/");
        File f = new File("C:\\Climate Monitoring"+File.separator+"Parametri"+File.separator+geonameid+"------"+dati_data[0]+dati_data[1]+dati_data[2]+"------"+centro+".txt");
        f.createNewFile();
        BufferedWriter bf = new BufferedWriter(new FileWriter(f));
